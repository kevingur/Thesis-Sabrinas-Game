...
from simulated_annealing import simulated_annealing, simulated_annealing_hybrid  
...
VARIANTS = [
    ("flip",   simulated_annealing,        "sa_results.csv",        False),
    ("hybrid", simulated_annealing_hybrid, "sa_hybrid_results.csv", True),
]
...
def _run_one(idx, variant_fn, is_hybrid, source, size, grid_id,
    ...
    if is_hybrid:
        (init, best, evals, runtime,
         final_coups, flips, rrs) = variant_fn(
            couplings, initial_obj, grid,
            max_evaluations=MAX_EVALS, Tstart=T_START, Tmin=T_MIN,
            p_riprepair=P_RIPREPAIR)
    else:
        (init, best, evals, runtime,
         final_coups) = variant_fn(
            couplings, initial_obj, grid,
            max_evaluations=MAX_EVALS, Tstart=T_START, Tmin=T_MIN)
        flips, rrs = None, None
