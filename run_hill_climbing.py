...
from hill_climbing import hill_climbing, hill_climbing_hybrid   
...
VARIANTS = [
    ("flip",   hill_climbing,        "hill_climbing_results.csv", False),
    ("hybrid", hill_climbing_hybrid, "hc_hybrid_results.csv",     True),
]
...
def _run_one(idx, variant_fn, is_hybrid, source, size, grid_id,
    ...
    if is_hybrid:
        (init, improved, improvements, evals, runtime,
         final_coups, sideways, flips, rrs) = variant_fn(
            couplings, initial_obj, grid,
            max_evaluations=MAX_EVALS, p_riprepair=P_RIPREPAIR)
    else:
        (init, improved, improvements, evals, runtime,
         final_coups, sideways) = variant_fn(
            couplings, initial_obj, grid, max_evaluations=MAX_EVALS)
        flips, rrs = None, None

    row = {"run_id": baseline_run_id, "source_algorithm": source,
           "grid_size": size, "grid_id": grid_id,
           "initial_total_objective_value": int(init), "improved_total_objective_value": int(improved),
           "improvements": improvements, "sideways": sideways,
